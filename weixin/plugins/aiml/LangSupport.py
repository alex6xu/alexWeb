...
GBSupport = type('GBSupport', (LangSupport, ),
        {'input' : lambda self, s:
                ''.join( findall_gb( type(s) == bytes and s.encode(self._encoding) or s ) ),
         'output': lambda self, s:
                sub_space(' ', sub_gb(r'\1', ( type(s) == bytes and s.decode('UTF-8') or s ).encode(self._encoding) ) ) } )

UnicodeSupport = type('UnicodeSupport', (LangSupport, ),
        {'input' : lambda self, s:
                ''.join( findall_utf8( type(s) == bytes and s.encode(self._encoding) or s ) ),
         'output': lambda self, s:
                sub_space(u' ', sub_utf8(r'\1', (type(s)==bytes and s.decode('UTF-8') or s) ) ) }
                     )
